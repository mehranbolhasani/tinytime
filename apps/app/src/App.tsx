...
import { Clock, CalendarDays, ClipboardList, Folder, Menu } from 'lucide-react'
  ...
  return (
    <main className="mx-auto flex min-h-screen w-full items-center justify-center bg-background px-4 py-10 sm:p-6">
      <div className="w-full max-w-sm flex flex-col items-center justify-center">
        <div>
          <div className="flex w-12 h-12 rounded-xl aspect-square items-center justify-center mx-auto bg-primary/10 mb-6 anim anim-scale anim-duration-700">
...
interface ThemePreferenceToggleProps {
  ...
  onChange: (option: ThemePreference) => void
}

function ThemePreferenceToggle({ preference, options, onChange }: ThemePreferenceToggleProps) {
  return (
...
function AppLayout({ userEmail, onSignOut, isSigningOut }: AppLayoutProps) {
...
export default function App() {
  ...
  return (
    <QueryClientProvider client={queryClient}>
      <MotionConfig reducedMotion="user" transition={{ duration: durations.base, ease: easings.standard as unknown as import('motion/react').Easing }}>
        <TimerProvider>
          <BrowserRouter>
            <AppLayout
